...
class DataProcessing:
    ...
    def build_from_csv(self, csv_path):
        ...
        dataframe = pd.read_csv(ROOT_PATH + csv_path)
        ...
        for idx, message in enumerate(all_mess):
            self.backup_text.append(message)
...
if __name__ == "__main__":
    ds = DataProcessing()
    X, y = ds.build_from_csv("./dataset/review_data.csv")

    print("Data size:", X.size())
    print("Label size:", y.size())

    torch.save(X, ROOT_PATH + "/dataset/train_data_v3.pt")
    torch.save(y, ROOT_PATH + "/dataset/train_label_v3.pt")
    torch.save(X[:600], ROOT_PATH + "/dataset/valid_data_v3.pt")
    torch.save(y[:600], ROOT_PATH + "/dataset/valid_label_v3.pt")